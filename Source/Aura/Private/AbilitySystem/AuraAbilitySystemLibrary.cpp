...
void UAuraAbilitySystemLibrary::InitializeDefaultAttributes(UObject* WorldContextObject, const ECharacterClass CharacterClass, float Level, UAbilitySystemComponent* ASC)
{
...
{
	// Only valid on the server - returns nullptr on clients
	const UCharacterClassInfo* CharacterClassInfo = GetCharacterClassInfo(WorldContextObject);
...
void UAuraAbilitySystemLibrary::GetLivePlayersWithinRadius(const UObject* WorldContextObject, TArray<AActor*>& OutOverlappingActors, const TArray<AActor*>& ActorsToIgnore, float Radius, const FVector& SphereOrigin)
{
	...
	// Query to see what we hit - we need the World object to detect overlaps
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		for (FOverlapResult& Overlap : Overlaps)
		{
			...
			if (IsAlive) OutOverlappingActors.AddUnique(Overlap.GetActor());
		}
	}
}
...
{
	// TODO: Implement a more robust system for determining friend or foe
